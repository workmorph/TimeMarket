...
async function login(): Promise<void> {
  ...
  try {
    ...
    console.error(
      chalk.red(`❌ ログインエラー: ${error instanceof Error ? error.message : String(error)}`)
    );
    process.exit(1);
  }
}
...
async function createApiKey(): Promise<void> {
  ...
  try {
    ...
    if (createdApiKey) {
      ...
      console.log(
        chalk.yellow(`  - 作成日: ${new Date(createdApiKey.created_at).toLocaleString()}`)
      );
      console.log(chalk.yellow(`  - APIキー値: ${apiKeyValue}`));
    }
  } catch (error) {
    console.error(
      chalk.red(`❌ APIキー作成エラー: ${error instanceof Error ? error.message : String(error)}`)
    );
    process.exit(1);
  }
}
...
async function listApiKeys(): Promise<void> {
  ...
  try {
    ...
    console.error(
      chalk.red(
        `❌ APIキー一覧取得エラー: ${error instanceof Error ? error.message : String(error)}`
      )
    );
  }
}
...
async function verifyApiKey(): Promise<void> {
  ...
  try {
    ...
    // テスト用のオリジンを設定
    const testOrigin = TEST_ORIGINS?.split(",")[0]?.trim() || "https://example.com";
    ...
    console.error(
      chalk.red(`❌ APIキー検証エラー: ${error instanceof Error ? error.message : String(error)}`)
    );
  }
}
...
async function deactivateApiKey(): Promise<void> {
  ...
  try {
    ...
    console.error(
      chalk.red(`❌ APIキー無効化エラー: ${error instanceof Error ? error.message : String(error)}`)
    );
  }
}
...
async function testErrorCases(): Promise<void> {
  ...
  try {
    ...
    const invalidKeyResponse = await fetch(`${BASE_URL}/api/verify-api-key`, {
      ...
      headers: {
        "Content-Type": "application/json",
        Origin: TEST_ORIGINS?.split(",")[0]?.trim() || "https://example.com",
      },
    ...
    if (!invalidKeyResponse.ok) {
      ...
      console.log(
        chalk.red(`❌ 不正なAPIキーが有効と判定されてしまいました: ${JSON.stringify(data)}`)
      );
    }
    ...
    if (!noNameResponse.ok) {
      ...
      console.log(
        chalk.green(`✅ 名前なしのAPIキー作成は適切に拒否されました: ${errorData.error}`)
      );
    } else {
    ...
    console.error(
      chalk.red(
        `❌ エラーケーステストエラー: ${error instanceof Error ? error.message : String(error)}`
      )
    );
  }
}

...
// CDNへのデプロイ
async function deployToCloudflare(version) {
  ...
  if (!CF_API_TOKEN || !CF_ACCOUNT_ID) {
    console.error('❌ Cloudflare認証情報が設定されていません。.env.localファイルを確認してください。');
    return false;
  }
  ...
  try {
    ...
    execSync(`wrangler r2 object put timebid-cdn/widget/${version}/ --path ${WIDGET_DIST_DIR}/* --recursive`, { 
      stdio: 'inherit',
      env: { 
    ...
    execSync(`wrangler r2 object put timebid-cdn/widget/latest/ --path ${WIDGET_DIST_DIR}/* --recursive`, { 
      stdio: 'inherit',
      env: { 
        CLOUDFLARE_API_TOKEN: CF_API_TOKEN,
        CLOUDFLARE_ACCOUNT_ID: CF_ACCOUNT_ID

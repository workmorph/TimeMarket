...
export default function WidgetPage() {
  ...
  if (isLoading) {
    return (
      <div id="widget-container" className="p-4 flex justify-center items-center h-[400px]" style={{ fontFamily: styles.fontFamily }}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center text-sm">
              ...
              <span>{auction.duration_minutes}分のセッション</span>
            </div>
            <div className="flex items-center text-sm">
              <DollarSign className="w-4 h-4 mr-1" />
              <span>開始価格: {formatCurrency(auction.starting_price)}</span>
            </div>
          </div>
                ...
                <div className="space-y-1 max-h-[120px] overflow-y-auto">
                  {bids.slice(0, 5).map((bid, index) => (
                    <div key={index} className="flex justify-between text-sm">
                      <span>入札者{bid.user_id.substring(0, 4)}...</span>
          ...
          <div className="text-xs text-center text-muted-foreground pt-2">
            Powered by <a href="https://timebid.jp" target="_blank" rel="noopener noreferrer" style={{ color: styles.primaryColor }}>TimeBid</a>
          </div>
        </CardContent>
      </Card>
    </div>

...
export async function GET(request: NextRequest) {
  // Note: requestパラメータは将来の拡張用に保持
  console.log("API Keys GET request:", request.url);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // バリデーション
    if (!body.name) {
      return NextResponse.json({ error: "APIキー名は必須です" }, { status: 400 });
    }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!keyId) {
      return NextResponse.json({ error: "APIキーIDが指定されていません" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "APIキーの無効化に失敗しました" }, { status: 500 });
  }
}

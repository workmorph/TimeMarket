...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!auction_id || !amount || isNaN(Number(amount)) || Number(amount) <= 0) {
      return NextResponse.json({ error: "入札金額が無効です" }, { status: 400 });
    }
    ...
    if (auction.status !== "active") {
      return NextResponse.json({ error: "このオークションは現在入札できません" }, { status: 400 });
    }
    ...
    if (now > endTime) {
      return NextResponse.json({ error: "オークションの入札期限が過ぎています" }, { status: 400 });
    }
    ...
    if (stripeEnabled) {
      ...
      if (!stripe) {
        return NextResponse.json({ error: "決済システムが利用できません" }, { status: 500 });
      }
    ...
    return NextResponse.json({ error: "入札の処理に失敗しました" }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filteredBids = bids.map((bid: Record<string, unknown>) => {
      ...
      return {
        ...
        profiles: {
          username:
            ((bid.profiles as { username?: string })?.username || "").substring(0, 1) + "***",
          avatar_url: (bid.profiles as { avatar_url?: string })?.avatar_url,
        },
      };
    });

...
// import { Badge } from '@/components/ui/badge' // 現在未使用
...
export function BidForm({ auction, onBidSuccess }: BidFormProps) {
  ...
  // 推奨入札額
  const suggestedBids = [minimumBid, minimumBid + 2000, minimumBid + 5000, minimumBid + 10000];
    ...
    return (
      <Card>
        <CardContent className="pt-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">このオークションは既に終了しています</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      <CardHeader>
        ...
        <div className="text-sm text-muted-foreground">
          <p>
            専門家: <span className="font-medium">{auction.expert.display_name}</span>
          </p>
          <p>
            現在の最高額:{" "}
            <span className="font-bold text-blue-600">{formatCurrency(currentHighest)}</span>
          </p>
        </div>
      </CardHeader>
